...
ts = time.time()
#################################################### 
cl = LINE("rkd93524@cuoly.com","a0970737883")
####################################################
cl.log(cl.authToken)
AuthToken = []
...
def kick(n, to, mid):
    while 1: 
        AuthToken[n].kickoutFromGroup(to, mid)
        break
def Runtime(secs):
...
def lineBot(op):
    try:
        ...
        if msg.contentType == 13:
                if settings["getmid"] == True:
                    if 'displayName' in msg.contentMetadata:
                        ...
                        cl.sendReplyMessage(msg.id, to, msg.contentMetadata["mid"])
                    else:
                        cl.sendReplyMessage(msg.id, to, msg.contentMetadata["mid"])
        if op.type == 5:#自動防封鎖
            print ("[ 5 ] INV PRO")
        ...
        if op.type == 25 and wait["um"]: 
        cl.unsendMessage(op.message.id)
        if op.type == 26 or op.type == 25:
            msg = op.message
            text = msg.text
            msg_id = msg.id
        ...
        if op.type == 26 or op.type == 25:
            ...
            if sender in admin:
				...
				#其餘加好友收回自動已讀
                    ...
                    anlian.sendReplyMessage(msg.id, to, "mid獲取開啟✔")
                elif text.lower() == 'getmid off':
                    settings["getmid"] = False
                    anlian.sendReplyMessage(msg.id, to, "mid獲取關閉✘")
