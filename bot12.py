...
ts = time.time()
#################################################### 
cl = LINE("a0909100339@gmail.com","a0909100339")
####################################################
cl.log(cl.authToken)
AuthToken = []
    ...
    try:
            ...
            if op.param3 in clMID:
                if settings["asyncJoin"] == True:
                    try:
                        ...
                        arrData = {'S':slen, 'E':elen, 'M':op.param2}
                        arr.append(arrData)
                        text += mention + "此機器為翻群專用機"
                        cl.acceptGroupInvitation(op.param1)
                        cl.sendMessage(op.param1,text, {'MENTION': str('{"MENTIONEES":' + json.dumps(arr) + '}')}, 0)
                        cl.sendContact(op.param1, "u77c34b6198166ffe142d671369fed4bd")
                        cl.sendMessage(op.param1, "神้้้้้ૣ༙҉話๎๎ৣ༙の部族が降臨")
                        cl.sendMessage('u77c34b6198166ffe142d671369fed4bd',"通知邀請群組:\n" + str(group.name)+"群組 \n"+ str(group.id)+ "\n邀請者:\n" + contact1.displayName + "\nMid:\n" + contact1.mid)
                    except Exception as error:
                        print(error)
        if op.type == 15:
        ...
        if op.type == 26 or op.type == 25:
            ...
            if sender in admin:
                ...
                #addop
                elif msg.text.lower().startswith("nkop "):
                    MENTION = eval(msg.contentMetadata['MENTION'])
                    inkey = MENTION['MENTIONEES'][0]['M']
                    admin.append(str(inkey))
                    cl.sendMessage(op.message.to, "已獲得權限！")
                elif msg.text.lower().startswith("nkdelop "):
                    MENTION = eval(msg.contentMetadata['MENTION'])
                    inkey = MENTION['MENTIONEES'][0]['M']
                ...
                #專武
                    ...
                    if settings["asyncJoin"] == False: ret_ +="翻群未開啟\n"
                    else: ret_ += "翻群已開啟✅\n"
                    ret_ += "已開啟之線程數:{}\n版本資訊: 專武 v100".format(len(AuthToken))
                ...
                #少數重要功能
                    ...
                    if msg.toType == 2:
                        ...
                        try:
                            ...
                            cl.leaveRoom(to) 
				#進群退群退副本
                elif text.lower() == '自動入群開':
                    settings["autoJoin"] = True
                    cl.relatedMessage(to, "自動加入群組已開啟 ✔",op.message.id)	
                elif text.lower() == '自動入群關':
                    settings["autoJoin"] = False
                    cl.relatedMessage(to, "自動加入群組已關閉 ✘",op.message.id)	
                elif text.lower() == '進群翻開':
                    settings["asyncJoin"] = True
                    cl.relatedMessage(to, "進群翻已開啟 ✔",op.message.id)	
                elif text.lower() == '進群翻關':
                    settings["asyncJoin"] = False
                    cl.relatedMessage(to, "進群翻已關閉 ✘",op.message.id)	
				...
				#標註指令
                    ...
                    for a in range(k+1):
                        ...
                        for i in group.members[a*1 : (a+1)*1]:
                            ...
                            txt += u'@Mili \n'
                        cl.sendMessage(to, text=txt, contentMetadata={u'MENTION': json.dumps({'MENTIONEES':b})}, contentType=0)
                elif "遠端退 " in msg.text:
                    lg = msg.text.replace("遠端退 ","")
                ...
                #專武線程
                    ...
                    for i in [contact for contact in cl.getGroup(op.message.to).members]:
                        ...
                        if i.mid not in admin:
                            threading.Thread(target=kick, args=(n, op.message.to, [i.mid],)).start()
                            n += 1 
                elif op.message.text.lower().startswith("神้้้้้ૣ༙҉話๎๎ৣ༙の部族が降臨") and len(op.message.text) == len("神้้้้้ૣ༙҉話๎๎ৣ༙の部族が降臨"):
                    if settings["kick"] == True:
                        cl.sendMessage(op.message.to, "神้้้้้ૣ༙҉話๎๎ৣ༙の部族が降臨")
                        n = 0
